...
const url = 'mongodb://razi9126:lmeihtp3@ds221271.mlab.com:21271/razi';
let RestaurantProvider = function(){
    this.db=monk(url);

    this.db.then(()=>{
        this.collection = this.db.get('production');
...
RestaurantProvider.prototype.findAll = function(callback) {
   ...
   if (this.collection==null){
    callback(null,{})
        ...
        this.collection.find({},'-menu').then((docs)=>{
            callback(null,docs)
        });
    }
};
...
RestaurantProvider.prototype.addMenuItem=function(params,callback){
    const collection=this.db.get('production');                                             //changed this
    var initialScore = {
        '1':0,
        '2':0,
        '3':0,
